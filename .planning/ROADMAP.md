...
- 13 reliability fixes, all on silent-failure paths — 7 from the audit, 6 from Codex review: pid-probing `acquireLock` (`process.kill(pid, 0)`; live/EPERM holders never reclaimed, ESRCH-dead and pid-less locks are), `classifyLocal` min-of-timeouts so the 60s-killed Stop hook passes 50000ms not 900s, one extraction-wide deadline in `memory-extractor` with a 2s floor and recorded `timeout` failure, `classifyAnthropic` honoring the SDK per-request `{ timeout }`, `oversizeThresholdBytes` (5 MiB) actually enforced with count-first chunk closing and byte-truncation of single oversize messages, `scripts/today-scheduled.js` exiting 1 on an error envelope, per-file `extraction-error` recording in `extractFromFile` (a throw abandons that file's remaining candidates, not the whole sweep), honest "N staged, M buffered" in `/wrap`, and a `LOAD_ERROR` decision log in `loadExcludedTerms` instead of a silent `[]` that disabled the exclusion gate.
- Vault graph restructured: 8-note `maps/` MOC layer — `maps/home.md` as the single entry point, 6 MOCs, and `how-to-read-the-brain-map.md` (the graph legend), 16 files triaged (moves only), 8 empty dirs removed, quarantine manifest added, LEFT-side `aliases:` frontmatter added under explicit operator authorization so 61 `memory.md` title-form links resolve.
- Memory store 183 → 285 entries: buffered backlog released by the lock fix and drained (98 promoted in 10 batches, 405 rejected and archived, 2 test fixtures left unresolved, 0 exclusion violations), plus a dream cycle (15 MERGE + 5 missed-pattern ADDs staged, 4 merges applied with the retrievability gate passing, 11 rejected with written reasons). `eval:recall` unchanged against `eval/baseline-2026-07-19.json` before and after.
...
- [ ] **Candidate: Memory Provenance Backfill (audit P1).** 16 of 285 `memory.md` entries carry neither `source-ref::` nor `added::` — the dream MERGE writer never emits them, so every merge widens the gap. Two parts: backfill the missing entries through the proposal gate, and patch the merge writer so it cannot happen again. Note the framing correction: only 4 of the 16 come from today's cycle — the other 12 date 2026-07-21, so they are older than all 98 entries promoted today. Prioritize this for the writer bug, not for the entries' recency. Related index hygiene, same pass: 22 lines still cite `_audit/2026-07-18`, a path that no longer exists (the P7 `memory-archive/` half is already fixed — the INDEX now points at the real `archive/memory`).
- [ ] **Candidate: /today Silent-Degradation Closure (audit P5).** Ten bare catch blocks in `src/today-command.js` swallow errors — the briefing group at 288/299/314/332/338 and the daily-stats group at 461/470/476/482/496, so a broken memory-health module produces normal-looking briefings forever. Smallest honest fix is a `degradedSections[]` line in the briefing. Same phase: `src/utils/classifier-health.js` increments the health file non-atomically across processes, and fallback-to-Haiku is invisible in instrumentation.
- [ ] **Candidate: Stop-hook extraction cannot use the local model (found 2026-07-31 by verification, not in the original audit).** The hook's 50s budget and the local model's ~86 tok/s cold prefill are two orders of magnitude apart: 50s buys ~4,300 prompt tokens against transcripts in the six figures, so every hook-driven extraction times out and falls back to Haiku. Evidence: `classifier-health.json` at `consecutive_failures: 6`, `last_failure_code: "timeout"`, `haiku_calls: 50/50` on 2026-07-31 while LM Studio served normally at 65,536 context. Options, cheapest first — (a) have the Stop hook request Haiku directly and reserve `provider: local` for terminal `/wrap` where the full 900s applies; (b) bound the hook's corpus to what 50s can actually prefill (~4k tokens) and accept partial extraction; (c) keep a warm prompt-cache path so prefill is the ~26s warm case rather than the ~9.5min cold one. Also worth fixing alongside: the Haiku cap being silently exhausted is itself invisible — nothing surfaces `haiku_calls: 50/50` to the operator.
