...
Last activity: 2026-07-31 - Audit & improvement pass (PR #96): 13 reliability fixes on silent-failure paths, vault `maps/` MOC layer, dream cycle (4 merges applied, gate passed), 483-candidate backlog drained (98 promoted / 405 rejected-and-archived)
...
- Repo: 13 reliability fixes on silent-failure paths (pid-probing `acquireLock`, min-of-timeouts in `classifyLocal`, extraction-wide deadline, SDK per-request timeout, `oversizeThresholdBytes` enforced, `today-scheduled.js` exit 1 on error envelope, per-candidate `extraction-error`, honest `/wrap` staged-vs-buffered counts, `LOAD_ERROR` on excluded-terms load failure). Details in MILESTONES.md.
- Vault: 7-note `maps/` MOC layer with `maps/home.md` as the entry point, 16 files triaged (moves only), 8 empty dirs removed, quarantine manifest added, LEFT `aliases:` frontmatter added under explicit operator authorization.
- Memory: 187 → 285 entries. Dream cycle applied 4 of 15 proposed merges with the retrievability gate passing; 11 rejected with written reasons in `proposals/dream-changeset-2026-07.md`. Backlog drain reviewed 504 candidates — 98 promoted in 10 batches, 405 rejected and archived, 0 exclusion violations.
