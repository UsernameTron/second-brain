...
- **Test count:** 1626 total across 85 test files (1588 passing, 38 skipped in CI)
- **Coverage:** Branch 80.48%, Statements 92.05%, Functions 95.53%, Lines 93.09% (measured 2026-09-08, local `CI=true` run, `coverage-summary.json total.*.pct`)
...
| `/pulse` | Weekly memory pulse over `memory.md` — what entered this week, what has been standing 45+ days, and one "is this still true?" question. Fully templated; nothing model-generated. Written Monday 07:00 by `com.secondbrain.pulse` to `briefings/pulse/`. `/pulse --show` prints the latest; `/pulse yes <hash>` confirms, `/pulse no <hash> [reason]` appends `stale::` (recall downranks it, nothing is deleted). The hash is required when more than one question is open |
