...
`DEPLOY_DRY_RUN=1 ./deploy/deploy.sh` stops before the Cloud Run update, but
**still performs earlier provisioning, IAM/secret operations, and an image build
and upload**. It is not a read-only approval preflight. Use
`./deploy/deploy.sh --selftest` for the offline parsing check. The
[2026-09-14 release review](releases/2026-09-14-ui.md) uses read-only inspection
and a local build before approval, then an image-only service update after
approval; it does not run the provisioning script.
