...
3. **Phase 2 (Claude-side HubSpot Agent CLI) — DONE, verified 2026-08-14.**
   All three env vars (`USE_ENVIRONMENT_HUBSPOT_CONFIG`,
   `HUBSPOT_PERSONAL_ACCESS_KEY`, `HUBSPOT_ACCOUNT_ID`) confirmed set in
   this environment. `hs project list --use-env` authenticated against
   account 243103424 and returned `No projects found` — correct, expected
   result (no projects uploaded yet). Along the way, corrected
   [HUBSPOT-AGENT-CLI.md](HUBSPOT-AGENT-CLI.md): its verification steps
   named `hs account info`, but that command has no `--use-env` flag at
   all and always requires a config-file default account — it can never
   verify env-based auth, with or without the flag. Doc now points at `hs
   project list --use-env` instead.
