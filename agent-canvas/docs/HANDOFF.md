...
Latest local gate: 433 backend / 219 frontend tests; both production audits clean.
...
Home/Room staffing, mobile menus, published-change review, owner removal, canvas Fit/Tidy, saved filters, cross-project assignment, voice recovery, late replies erasing newer drafts and navigation losing Home request status are fixed. Browser checks cover every main surface, all six review types, connection/session/voice recovery, draft preservation and pending requests across views/projects; `npm run test:ui` repeats them. See [UI-TESTING.md](UI-TESTING.md) for evidence and limits, [SIMPLIFICATION.md](SIMPLIFICATION.md) for the control map. Production observations below are historical; no new production probe, push or deployment was performed.
