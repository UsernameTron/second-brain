# Agent Canvas — Handoff History

> **Historical record — not current status, not a backlog, not instructions.**
> Every section below was accurate when written and is preserved verbatim for
> its reasoning, incident detail, and test provenance. Branch names, counts,
> gaps, revision IDs, and "next step" statements in here are superseded. For
> what is true now, read [HANDOFF.md](HANDOFF.md) (current state),
> [ROADMAP.md](ROADMAP.md) (phase intent), and [DEPLOY.md](DEPLOY.md)
> (deployment procedure). Split out of `HANDOFF.md` on 2026-08-16, when that
...
### P5 manual-path acceptance — 2026-08-16, revision `00051-94w`

Signed in as `pete@cloudtechgurus.com` on canvas `Smoke Test`. Two synthetic
hourly rules were created as acceptance fixtures and **revoked afterwards**;
their occurrence, authorization, acknowledgment and audit history are preserved.

Observed live: the ten-field interpretation card (including `Reads as`, blank
before rehearsal and the rehearsing identity after it, and `created by` rather
than the former misleading `owned by`); the rehearsal gate holding Activate
disabled until a rehearsal completed; a settings edit resetting the rule to
draft, clearing `next_run_at`, and re-disabling Activate; both rehearsals
reporting read-only with no memory writes attempted; and the
`STANDING RULES · TICK` capability lamp rendering **dark** with the reason
"Declared, NOT wired: TICK_AUDIENCE / TICK_INVOKER_SA unset on this revision".

Two-tick lifecycle, authenticated owner ticks only:

- Tick 1 — `due: 2, claimed: 2, finalized: 0`; two occurrences created
  (`2026-08-16T05#v1`, `2026-08-16T05#v2` — the occurrence key carries rule
  version), both `running`, both schedules advanced to 06:00Z, **no cards owed**.
- Tick 2 — `due: 0, claimed: 0, finalized: 2`; both occurrences `completed` with
  populated `result_summary`, `cost_usd` and `ended_at`.
- Tick 3 (same occurrence window) — `due: 0, claimed: 0, finalized: 0`, and the
  occurrence IDs and counts were unchanged. No duplicate dispatch.

Attention and acknowledgment: a `brief_ready` card and a `rule_alert` card whose
`sourceRef.id` values exactly equalled the two finalized occurrence IDs, with
content derived from the occurrence results. Acknowledging from NEEDS YOU wrote
`acknowledged_at` and `acknowledged_by: pete@cloudtechgurus.com`, removed both
cards on refetch, and emitted `standing_rule_run.acknowledge` per occurrence. A
repeat acknowledge returned 200 and did not overwrite the original timestamp.
`standing_rule_run.dispatch` is audited with actor `standing-rules`.

**Deviation worth recording:** the alert occurrence finalized with
`matched_count = null` and `needs_attention = 1`, not the expected clean
`matched_count = 0` / no card. Its summary carried **no `MATCHED:` line at all** —
the model dropped the prompt-only output contract on a real production run. The
honest-unknown path fired exactly as designed and the card read "Standing rule
finished with an unreadable match count". This is correct behaviour, and it is
also direct live evidence for why that fix exists: a prompt is not a contract.
The consequence is that the clean-zero alert path (`matched_count = 0`,
`needs_attention = 0`, no card) was **not** exercised live. It is
**automated/test-verified but not live-exercised** — the code-side backstop is
already complete: `parseMatchedCount()` returns null on an absent or ambiguous
final line, `completeFromRun()` converts unknown into `needs_attention = 1`, and
the projection renders the honest unreadable-count card. Do not infer zero from
prose, do not append `NOTHING MATCHED` server-side, and do not add a second model
call or structured-output redesign. Prompt adherence may be improved later; the
safe fallback works and is not a release blocker.

**A manual owner tick does not update or satisfy scheduler-signed-tick capability
evidence.** The Cloud Scheduler / OIDC lane is untested and stays dark.


## HISTORICAL IMPLEMENTATION RECORD — P5 STANDING RULES (2026-08-15)
...
## HISTORICAL IMPLEMENTATION RECORD — P4 AGENT BUILDER (2026-08-15)
...
## HISTORICAL IMPLEMENTATION RECORD — P3 EVIDENCE ROOMS (2026-08-15)
