...
|---|---|---|---|---|
| 2026-08-14 | local exec | Radar smoke | DONE (behavior) / cost caveat | Radar run on `agent-canvas-00032-ss4` returned **100 hot leads, all ≥0.75**, each with per-lead arithmetic (`why`), version-stamped `sr-icp-v5`, 3 memory entries logged — and it did NOT spin (last time: 5 polls in 9s). The behavior fixes work, and the run's shape proves `reseedRosterPrompts` propagated the new prompt to the live roster row. **But it was not cheap:** ~$1.56 / ~280k input tokens for this run (cumulative panel $2.48 / 457k over 2 runs; run 1 was $0.92 / 178k). This run cost MORE than the burn because it *succeeded* and re-sent the growing 100-lead payload across its poll turns with no prompt caching — the token amplifier the survey flagged (IMPROVE-FINDINGS finding-8-adjacent: uncached calls + monotonic message array + full tool-schema every call). Cutting it needs prompt caching / message trimming, a separate change. |
| 2026-08-14 | local exec | GTM + #130 | RESOLVED — credential, confirmed by Pete's audit | Pete's GCP findings register (F-00, CLOSED) settles it: the local gcloud credential *labeled* `pete@cloudtechgurus.com` held **cpeteconnor@gmail.com's refresh token** — `gcloud auth list` and `CLOUDSDK_CORE_ACCOUNT` both report the label, not the token; only `tokeninfo` exposed it (exactly the check this session's lesson mandates). So my #130 "perimeter" was the crossed credential, and pete@ **held dataset-level BigQuery Data Owner on `ctg_gtm_marts` the whole time** — his hypothesis was right, GTM was never blocked. Credential re-authed; `bq show ctg_gtm_marts` succeeds; **GTM UNBLOCKED, no IAM change needed.** The GTM bridge can now be built against the real schema. |
| 2026-08-15 | local exec + Pete in-app | 2 (SOI half) | **DONE** | Full bar passed. `soi-mcp` deployed (`soi-mcp-00001-jk2`), route merged upstream (ctg-system-of-intelligence#5). Pete wired the connector row and ticked `org_knowledge_search` (the tick is the sign-off); probe 71ms. Corpus-MISS verified twice by live agent runs — honest "Not found in the corpus.", results wrapped as `external_content`, no invention. Corpus-HIT verified live via an SA-impersonated MCP call: grounded answer with `[CTG_LinkedIn_Content_Plan.pdf]` citations on a gold-set question (the M01 corpus holds firm-overview/roster/priorities/content-strategy docs — V/TO and commission governance are NOT indexed, which is why those miss honestly). `asked_by` confirmed server-controlled (#163): the model sent `scout@`, the wire carried the directing user. **One deviation:** the row was created `access: members`; the runbook holds it `owner` until usage data (~60 req/min RAG ceiling, no limiter, 2 Gemini calls/answer) — Pete to flip the dropdown. First probe cold-started past the 30s MCP timeout; the retry is the fix. |
| 2026-08-15 | local exec | fly.dev v6 re-export (B2) | **DONE** | `sr-icp-connector` re-deployed with the regenerated `export_icp.py` v6 artifact (byte-identical to the canvas copy); live `ping` returns `sr-icp-v6`, 3 tools intact, poll path answers. The "v5-scored" caveat in older rows below is closed; Radar's prompt (reseed v5, revision `agent-canvas-00045-gwg`) checks the live version instead of naming one and states an honest fallback when `ping` is unticked. |
| 2026-08-14 | local exec | enrichment lane | READY — but see security decision | Pete granted `agent-canvas-run@` service-scoped Cloud Run Invoker on `enrichment-dispatch` and confirmed ingress=All; URL `https://enrichment-dispatch-874411154198.us-central1.run.app`. Lane is one `ED_DISPATCH_URL` + redeploy from live. **HELD pending Pete's call:** his own findings register (F-01/F-07) flags the agent-canvas→`enrichment-dispatch`→`ctg-hs-exec-tool` chain as the exposure that "reached production," and F-01 (external gmail identity w/ prod admin) + F-02 (DRS off) are open during funding diligence. Lighting a new production ingress mid-remediation is the wrong direction; recommend holding until F-01 closes. Not lit unilaterally. |
| 2026-08-14 | local exec | Radar + mechanism fixes | READY-TO-DEPLOY | The $0.92 polling burn fixed at three layers. **Radar prompt:** hot-leads-only (`min_score` 0.75, drop the rest), every lead carries its `why` breakdown, bounded poll (1 check + ≤2 `wait`s then park), honest `incomplete` when the async job outlives the run. **Two agent-generic mechanism fixes:** a `wait` tool (the server's only delay primitive, ≤30s, abortable by global pause) so the connector's "wait 60s and check" contract is followable at ~2 calls not 8; and `complete` gains an `outcome: incomplete` that maps to the existing `failed` status + a tray escalation — closing the bug where a run that gave up filed itself as `completed`. A genuinely-silent run (no text, no memory write) also stops reading as success. **Prompt propagation:** `seed_roster_prompts_v2` guarded re-seed + generated `legacy-roster-prompts.json` (a plain prompt edit never reached the live DB — `seedRoster` is one-shot). 130/130. Deferred: redeploy + one Radar run confirming ≤2 calls and an honest status. |
| 2026-08-14 | local exec | #130 CORRECTION | DONE (doc) | **PR #130's "VPC-SC perimeter" is WITHDRAWN.** `tokeninfo` proves this Mac's gcloud mints a **`cpeteconnor@gmail.com`** token even for `--account pete@cloudtechgurus.com` — both accounts resolve to gmail; a pete@ token is not obtainable here. So every "as pete@" probe behind #130 ran as gmail, which holds nothing on these org projects: "403 on every service API" is gmail-not-authorized, not a perimeter, and pete@'s real access was never tested from the CLI. HANDOFF open item 8 rewritten; lesson recorded in `tasks/lessons.md`. Second time the two-identity trap bit this session. |
| 2026-08-14 | local exec | 1 (GTM) | BLOCKED — **UNTESTED**, not diagnosed | Supersedes the "diagnosed" and "correction" rows below. GTM is neither confirmed nor refuted: Pete's hypothesis (dataset-level `dataOwner` grants `datasets.get` but not project-scoped `datasets.list`, so console and API answer different questions) is **unrefuted** — my `datasets.get` 403 that seemed to refute it was gmail, per the #130 correction. The one call that settles it must run as pete@: `bq show ctg-hs-exec-tool:ctg_gtm_marts` in Cloud Shell. Project confirmed `ctg-hs-exec-tool` by breadcrumb + Dataset ID. |
| 2026-08-14 | local exec | 1 (GTM) | correction | While attempting the GTM grant, a **project-level** `roles/bigquery.dataViewer` was briefly added to `ctg-hs-exec-tool` — the exact shortcut-grant the spec forbids, since it reads `ctg_gtm_raw` (44k people's emails/phones). Removed within the minute and absence verified. No data was read: every BigQuery call in this session returned Access Denied. The dataset-scoped grant the spec names was then attempted and is itself denied by the perimeter. |
| 2026-08-14 | local exec | 1 (sr-icp half) · Hardening · 3 (harvest half) | **DONE** | Full bar passed. `npm test` 124/124 · deployed **`agent-canvas-00031-mpk`** · connector probe `hubspot-crm` **85ms / 14 tools**, `sr-icp-leadfinder` **3 tools** enabled by the owner · agent smoke run end-to-end on `read_registry` with its audit entry confirmed in Cloud Logging (`run.start` → `run.dispatch` → `run.finish`, `stepsUsed: 2`, `costUsd: 0.164229`, status completed) — the agent returned 25 of 28 supplier matches, honored the truncation notice, and cross-referenced `org_context` unprompted · lamps honest (amber while the stored connector config still disagreed with the enforced one; green once it converged) · docs updated in-PR. Two live defects found and fixed on the way, each with its own PR: **#127** a refused write tool was reporting itself as a fatal config-parse failure that claimed no connector was active; **#128** the admin route rejected any save containing a write tool, so the save that removed them was refused for containing them — it strips and reports now. |
| 2026-08-15 | local exec | 1 (sr-icp smoke) | **DONE** | Full async start/poll pair exercised live against `sr-icp-connector.fly.dev`: `find_icp_leads` (Healthcare, min_score 0.75) → job `c1613ff9337e` → `check_lead_search` polled through `searching companies` → `searching people` → **done, 10 leads**, every lead carrying its `why` arithmetic (category/tier/tier_multiplier/industry_weight/seatband/exec_level/revenue), all tier-1 Healthcare decision-makers, score 1.15. **`icp_version: sr-icp-v6`** on the result — confirms the v6 re-export end-to-end, closing the v5-skew caveat this row used to carry. |
| 2026-08-14 | local exec | 1 (sr-icp smoke) | ~~READY-TO-DEPLOY~~ SUPERSEDED by the DONE row above | The row and its 3 tools are live and probed, but no agent run has exercised `find_icp_leads`/`check_lead_search` yet. One targeting-role run that starts a search and polls it closes this. Note for whoever writes it: they are an async start/poll pair, and results are **v5-scored** until the fly.dev service is re-exported. |
| 2026-08-14 | local exec | deploy | PARTIAL — server half of the bar passed | **Deployed: revision `agent-canvas-00029-tr6`**, carrying #122/#123/#124. Env survived the wholesale set (`HS_OPS_RUNNER_URL`, `MODEL_PROVIDER`, `GOOGLE_CLIENT_ID`, `RAPIDAPI_KEY`, `OWNER_EMAIL`, `NODE_ENV` all present). Verified live from the logs and the unauthenticated config endpoint: (1) the seed audit line reads `{"candidates": 3, "servers": 1}` — the `seed_mcp_v1`→`v2` upgrade landed **exactly the one new row** on the already-seeded workspace, and the audit-honesty fix reported the truth rather than the constant's length; (2) `/api/config` returns **`devAuth: false`** in production, so #124's `DEV_AUTH` backstop holds on the live service. **Still owed, and only Pete can do them** (they need a signed-in session): the connector probe for `sr-icp-leadfinder` (expect 3 tools + latency), a re-probe of the existing connectors to see whether #124's connector rule refused anything, one agent smoke run with its audit entry, and the lamp check. Rows stay `READY-TO-DEPLOY` until those pass. |
| 2026-08-14 | local exec | — | ~~PORTFOLIO NOT COMPLETE — nothing deployed~~ SUPERSEDED by the DONE row above | Every wave in the Revised wave order was worked and every one is `READY-TO-DEPLOY` or `BLOCKED`; **zero rows reached `DONE`, because no row can.** The deploy is the common blocker: `./agent-canvas/deploy/deploy.sh` was refused by this session's permission layer, so nothing built here has reached Cloud Run and no connector probe, agent smoke run, or lamp check has been performed. Four PRs merged: **#121** Step-1 review · **#122** Wave 1 sr-icp row · **#123** Wave 2 ED lane + SOI runbook · **#124** hardening + context registries. `npm test` 123/123 (was 95). **The next local session's first duty** is the deferred verification bar for every `READY-TO-DEPLOY` row, in the order below. Rows left `BLOCKED`: GTM bridge (BigQuery grant), SOI deploy (`ctg-workspace-dev` CLI), Wave 3 upstream v6 (`gh auth switch`), Apps Script parity (needs a live run). Parked items stay parked; no Wave 4 was invented. |
...
| 2026-08-15 | local exec | 3 (Apps Script parity) | **4/5 DONE — Workflows open** | Ran headlessly against the REAL portal 243103424 via the canvas SA impersonated on the `hubspot-mcp-bridge` audience (the connector's exact path). Q2 data-trust, Q3 ownership (2 owners = 89% of 1102 deals), Q4 open-by-category (291 open across ~12 `product_category` values), Q5 relationships (Supplier/Referral-Partner assoc defs) — all PASS with live numbers. **Q1 Workflows FAILS: token lacks `automation` scope, so `hubspot-list-workflows`/`hubspot-get-workflow` 403.** Full checklist + the Q1 decision (recommend untick the two workflow tools per the "no tool the deployment can't honor" rule; keep the /exec Workflows button) in PORTFOLIO-FOLD-IN.md § parity. **Retire the four verified /exec buttons; do NOT retire Workflows** until Q1 is decided. |
| 2026-08-14 | local exec | 3 (Apps Script parity) | ~~BLOCKED~~ SUPERSEDED by the 4/5 row above | Assigned to Wave 3 by the Step-1 review. Needs a live canvas run answering each of the exec-tool page's five locked-button questions — impossible while the deploy is blocked. |
