...
|---|---|---|---|---|
| 2026-08-14 | local exec | 2 (ED) · 1 (GTM) · 2 (SOI) | BLOCKED — **one root cause, diagnosed** | All three attempted directly and all three denied. Evidence: pete@ holds **`roles/owner` + `roles/run.admin`** on `ctg-hs-exec-tool` and **`roles/owner`** on `ctg-workspace-dev`, yet is denied **every service API** on them (Cloud Run, Service Usage, Secret Manager, Logging, IAM, Storage, BigQuery) while **Resource Manager IAM works** — `projects add-iam-policy-binding` successfully wrote a binding on the same project in the same minute. Owner cannot be denied `run.services.list` by IAM, so this is a **VPC-SC perimeter or org deny policy**, not a role gap, and no grant fixes it. One console session clears all three; Cloud Shell would prove the perimeter conclusively. Supersedes HANDOFF open item 8's earlier guess. |
