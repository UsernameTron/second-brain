...
| 2026-08-15 | local exec + Pete in-app | 3 (Apps Script parity) | **DONE** | Q2–Q5 verified headlessly against the REAL portal 243103424 via the canvas SA impersonated on the `hubspot-mcp-bridge` audience (the connector's exact path): data-trust, ownership (2 owners = 89% of 1102 deals), open-by-category (291 open across ~12 `product_category` values), relationships (Supplier/Referral-Partner assoc defs) — all PASS with live numbers. Q1 Workflows had failed (token lacked `automation` scope, so `hubspot-list-workflows`/`hubspot-get-workflow` 403); **Pete resolved Q1 in-app 2026-08-15** (config-side, not headlessly re-verifiable). Full checklist in PORTFOLIO-FOLD-IN.md § parity. Parity closed. |
